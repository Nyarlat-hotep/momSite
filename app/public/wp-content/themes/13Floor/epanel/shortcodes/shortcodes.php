...
define('ET_SHORTCODES_VERSION', '1.6');
define('ET_SHORTCODES_DIR', get_bloginfo('template_directory') . '/epanel/shortcodes');
...
function et_imagetab($atts, $content = null) {
	...
	$output = "
		<div{$id} class='et-image{$class}' style='background: url(".get_bloginfo('template_directory') . "/timthumb.php?src={$content}&amp;w={$width}&amp;h={$height}&amp;zc=1) no-repeat; width: {$width}px; height: {$height}px;'><span class='et-image-overlay'> </span></div>";
...
function et_testimonial($atts, $content = null) {
	...
	$image_markup = '';
	if ( $image <> '' ) {
		$image = ( $timthumb == 'on' ) ? get_bloginfo('template_directory') . "/timthumb.php?src={$image}&amp;w=57&amp;h=57&amp;zc=1;" : $image;
		$image_markup = "
			<div class='t-img'>
