<?php	
	...
	$featured_num = get_option('13floor_featured_num');
	...
	if (get_option('13floor_use_pages') == 'false') query_posts("showposts=$featured_num&cat=".get_catId($featured_cat));
		...
		query_posts(array
						...
						'orderby' => 'menu_order',
						'order' => 'ASC',
						'post__in' => get_option('13floor_feat_pages'),
	...
	<div id="featured-area">
			...
			<?php for ($i = 1; $i <= $featured_num; $i++) { ?>
				...
				<div class="slide">
					<div class="description">
						<h2 class="title"><a href="<?php echo($arr[$i]["permalink"]); ?>" title="<?php printf(__('Permanent Link to %s', '13floor'), $arr[$i]["fulltitle"]) ?>"><?php echo($arr[$i]["title"]); ?></a></h2>
						<p><?php echo($arr[$i]["excerpt"]); ?></p>
						
						<a href="<?php echo($arr[$i]["permalink"]); ?>" class="readmore"><span><?php if($arr[$i]["button_text"] <> '') echo($arr[$i]["button_text"]); else echo(__('Buy Now!', '13floor')); ?></span></a>
					</div>
					
					<a href="<?php echo($arr[$i]["permalink"]); ?>" title="<?php printf(__('Permanent Link to %s', '13floor'), $arr[$i]["fulltitle"]) ?>">
		...
		<div id="controls" class="clearfix">
			<a href="" id="prevlink"><?php _e('Prev','13floor'); ?></a>
			<a href="" id="nextlink"><?php _e('Next','13floor'); ?></a>
